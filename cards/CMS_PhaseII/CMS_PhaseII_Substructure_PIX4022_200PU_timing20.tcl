...
module TrackPileUpSubtractorWithTiming TrackPileUpSubtractorWithTiming {
# add InputArray InputArray OutputArray
  add InputArray HCal/eflowTracks eflowTracks
  add InputArray ElectronFilter/electrons electrons
  add InputArray MuonMomentumSmearing/muons muons
  ...
  # assume perfect pile-up subtraction for tracks with |z| > fZVertexResolution
  # Z vertex resolution in m
  set TVertexResolution {((abs(eta)<3. || abs(eta) > 4.0)*99999.0 + (abs(eta)>=3. && abs(eta)<=4.0)*20e-12)} 
